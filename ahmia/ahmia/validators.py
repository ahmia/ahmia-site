...
def validate_onion_url(url):
    ...
    if url[0:7] != 'http://':
        raise ValidationError(_(u'{url} is not beginning with http://', url=url))
    if url[-7:] != '.onion/':
        raise ValidationError(_(u'{url} is not ending with .onion/', url=url))
    if not validate_onion(url[7:-7]):
        raise ValidationError(_(u'{url} is not valid onion domain', url=url))
